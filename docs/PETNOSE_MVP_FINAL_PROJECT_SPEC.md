...
Auth support table은 아래 1개다.

8. `password_reset_tokens`

`password_reset_tokens`는 domain table이 아니라 auth support table이며 reset token 원문 대신 SHA-256 hash만 저장한다.
...
과거 separate profile, auth history, report, refresh token, image quality/crop, reservation, payment, contract, report/admin dashboard, non-canonical role extension 영역은 current canonical MVP v2 schema 또는 API surface에 포함하지 않는다.

아래 항목은 current MVP active scope가 아니다.

- `post_adoption_verifications` table
- 입양 후 1주/3개월/6개월 주기적 비문 인증
- `ADOPTER` role
- `SHELTER` role
...
사용자 profile image는 optional `profile_image_*` metadata fields로 관리한다. `profile_image_path`는 upload root 기준 상대 경로이고 API의 `profile_image_url`은 `/files/{relative_path}`로 계산한다.

## Password Reset Tokens

`password_reset_tokens`는 auth support table이다. Domain/relationship table count에는 포함하지 않는다.
...
Reset token 원문은 저장하지 않는다. `token_hash`는 SHA-256 hex hash이며 password reset request/confirm flow의 server-side 검증에만 사용한다.
...
`COMPLETED` 전이는 owner-only status update action으로 처리한다. 이때 `adopter_user_id`는 active `users.id` reference로 required이고, `adopted_at`은 완료 처리 시각이다. `dogs.owner_user_id`는 등록자/작성자 ownership으로 유지하며 입양자로 변경하지 않는다.
...
`adoption_post_likes`는 좋아요/찜 상태를 저장하는 relationship table이다. `users.liked` JSON/map을 사용하지 않는다.
...
`user_id`와 `post_id` 조합은 unique다. 좋아요 상태의 source of truth는 MySQL `adoption_post_likes`다.
...
4. `registration_allowed=false`인 duplicate suspicion decision은 adoption post creation을 막고 Qdrant upsert를 수행하지 않는다. `REVIEW_REQUIRED`는 호환 값으로 남지만 active normal registration decision에서는 반환하지 않는다.
