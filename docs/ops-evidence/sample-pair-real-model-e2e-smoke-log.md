...
curl.exe -sS -w "`nHTTP_STATUS:%{http_code}" -X POST "http://localhost:8080/api/dogs/register" -H "Authorization: Bearer <JWT-redacted>" -F "name=MilkDog" -F "breed=Mixed" -F "gender=UNKNOWN" -F "nose_image=@<sample:milk1>;type=image/png" -F "profile_image=@<sample:milk_Profile>;type=image/png"
...
curl.exe -sS -w "`nHTTP_STATUS:%{http_code}" -X POST "http://localhost:8080/api/dogs/register" -H "Authorization: Bearer <JWT-redacted>" -F "name=NoseDog" -F "breed=Mixed" -F "gender=UNKNOWN" -F "nose_image=@<sample:nose3>;type=image/jpeg" -F "profile_image=@<sample:profile2>;type=image/png"
...
| milk1 registration | `POST /api/dogs/register` | `milk1`, `milk_Profile` | HTTP 201 registered | 201 | `registration_allowed=true`, `status=REGISTERED`, `verification_status=VERIFIED`, `embedding_status=COMPLETED`, `qdrant_point_id=dog_id`, model real, dimension 2048 | PASS | file serving HTTP 200 for returned nose/profile URLs |
...
| nose3 registration | `POST /api/dogs/register` | `nose3`, `profile2` | HTTP 201 registered | 201 | `registration_allowed=true`, `status=REGISTERED`, `qdrant_point_id=dog_id`, model real, dimension 2048 | PASS | Qdrant count `2 -> 3` |
